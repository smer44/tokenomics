...
- Агент распределитель - распределяет токены пользовательских заказов между "мощностями" (или агентами производителями????)
  - Если после ставок на бирже закзы не были взяты в работу то токене возвращаются потребителям и сберегаются (ВОПРОС: могут ли потребители потратить токены на другие продукты в этом же раунде? Вероятно нет)
- Агент производитель -
